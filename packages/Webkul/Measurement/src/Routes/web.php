<?php

use Illuminate\Support\Facades\Route;
use Webkul\Measurement\Http\Controllers\MeasurementFamilyController;
use Webkul\Measurement\Http\Controllers\MeasurementUnitController;
use Webkul\Measurement\Http\Controllers\AjaxController;
use Webkul\Measurement\Http\Controllers\AttributeController;
use Webkul\Measurement\Http\Controllers\MeasurementFamilyOptionsController;
use Webkul\Measurement\Http\Controllers\MeasurementController;

Route::group(['middleware' => ['web', 'admin'], 'prefix' => 'admin/measurement'], function () {

    // Measurement Families
    Route::controller(MeasurementFamilyController::class)
        ->prefix('families')
        ->group(function () {
            Route::get('/', 'index')->name('admin.measurement.families.index');
            Route::get('/create', 'create')->name('admin.measurement.families.create');
            Route::post('/', 'store')->name('admin.measurement.families.store');
            Route::get('/{id}/edit', 'edit')->name('admin.measurement.families.edit');
            Route::put('/{id}', 'update')->name('admin.measurement.families.update');
            Route::delete('/{id}', 'destroy')->name('admin.measurement.families.delete');
            Route::post('/mass-delete', 'massDelete')->name('admin.measurement.families.mass_delete');

            Route::get('measurement-families/{id}/units', 'units')->name('admin.measurement.families.units');
            Route::post('{id}/units', 'storeUnit')->name('admin.measurement.families.units.store');
            Route::get('measurement-families/{familyId}/units/{code}/edit', 'editUnit')->name('admin.measurement.families.units.edit');
            Route::put('measurement-families/{familyId}/units/{code}/update', 'updateUnit')->name('admin.measurement.families.units.update');
            Route::delete('measurement-families/{familyId}/units/{code}', 'deleteUnit')->name('admin.measurement.families.units.delete');
            Route::delete('measurement-familiesddd', 'unitmassDelete')->name('admin.measurement.families.unitmass_delete');
        });


    Route::get('get-families', [MeasurementController::class, 'getFamilies'])->name('get-families');
    Route::get('get-units', [MeasurementController::class, 'getUnits'])->name('get-units');


    Route::get('ajax/families/{id}/units', [\Webkul\Measurement\Http\Controllers\AjaxController::class, 'units'])
        ->name('measurement.ajax.units');
    Route::get('measurement/families', [AttributeController::class, 'customFieldData'])->name('measurement.families');
    Route::get('measurement/custom-field-data', [AttributeController::class, 'customFieldData'])
    ->name('measurement.custom-field-data');

    Route::get('measurement/familiesss', [AjaxController::class, 'families'])
    ->name('admin.measurement.families');

Route::get('measurement/units', [AjaxController::class, 'units'])
    ->name('admin.measurement.units');

   Route::get('attributes/custom-field-data', [AttributeController::class, 'getAttributeMeasurement'])
    ->name('admin.attributes.custom-field.data');

});
